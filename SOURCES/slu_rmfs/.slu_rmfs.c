 ...
 * A production implementation would probably require a 9p, NFS or VNFS implementation.
 */
...
main(int ac, char **av) {
  ...
  FILE                         *prev_ctlfile, *already_mounted(void);
  tri_t                         request_unmount(FILE *, long);

  extern struct fuse_args       f_args; /* fuse_linkage.h */
  ...
  ingest_config(INITIAL_START);
  mk_fs();
  
  if ((prev_ctlfile = already_mounted())) {
    request_unmount(prev_ctlfile, 0);
  }
  ingest_config(PREV_GENERATION);
